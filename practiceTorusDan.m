function practiceTorusDan (rIn, rOut)
%File namer
if mod(rIn,1)~=0
...
filename = strcat('practiceTorusDan rIn',rInString,'um rOut',rOutString,'um.gwl');
...
%Define laser power
power_pSiJ400_6_29_18 = [0,10,15,20,25,30,35,40,45,50,55,60,65,70];
n_pSiJ400_6_29_18 = [1.4263,1.472,1.536,1.645,1.675,1.71,1.863,1.945,1.95,1.96,1.975,1.987,1.99,2.05];
n = 1.863;
lp = interp1(n_pSiJ400_6_29_18, power_pSiJ400_6_29_18, n);

nMatrix = n_pSiJ400_6_29_18;
lpMatrix = power_pSiJ400_6_29_18;
nMin = min(nMatrix);
nMax = max(nMatrix);
lpMin = min(lpMatrix);
lpMax = max(lpMatrix);
...
for z = zMin : spacing : zMax
    ...
    for r = rStart : spacing : rEnd
        for theta = 0 : dTheta : pi - dTheta
            ...
            y0 = r * sin(theta0);
            yf = r * sin(thetaf);
            xC = rCenter * cos(theta);
            yC = rCenter * sin(theta);
            rGRIN = abs(sqrt((r - rCenter)^2 + z^2));
            n = nMax - (delN * (rGRIN / rTor));
            if n<nMin
                n=nMin;
                disp('too low')
            elseif n>nMax
